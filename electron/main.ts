...
ipcMain.on('auth:get-fresh-credentials', async (event, { existingCookieId = null }) => {
    ...
    try {
        ...
        const credentials = await new Promise<{ value: string; bearerToken: string; email: string | null }>(async (resolve, reject) => {
            const timeout = setTimeout(() => reject(new Error('Hết thời gian chờ lấy thông tin xác thực.')), 35000);
            ...
            // SỬA LỖI: Sử dụng phương pháp điều hướng đi và quay lại để kích hoạt request
            try {
        ...
        setTimeout(() => authBrowser.close(), 2000);
...
// =================================================================
// CÁC HÀM TIỆN ÍCH KHÁC VÀ LOGIC AUTOMATION
// =================================================================
    ...
    try {
        ...
        try {
            const newProjectResponse = await handleApiRequest(null, {
                ...
                cookie: activeCookie,
                options: { method: 'POST', body: { json: { projectTitle: `Veo Project Auto - ${new Date().toLocaleString()}`, toolName: "PINHOLE" }}}
            });
...
// =================================================================
// HÀM TẠO CỬA SỔ CHÍNH VÀ VÒNG ĐỜI ỨNG DỤNG
// =================================================================
