...
import { Story, VideoPrompt, GeneratedImage, GeneratedVideo, YouTubeScript, UserCookie, LabsProject, AutomationState, AutoSaveConfig } from '../types';
...
export const AppProvider: React.FC<{children: ReactNode}> = ({ children }) => {
    ...
    // Lắng nghe sự kiện từ backend để tự động lưu/cập nhật cookie
    useEffect(() => {
        // SỬA LỖI: Đổi tên hàm thành "onCredentialsReady"
        const unsubscribe = window.electronAPI.onCredentialsReady((data) => {
            if (data.existingCookieId) {
                // Trường hợp CẬP NHẬT
                updateCookie(data.existingCookieId, { value: data.value, bearerToken: data.bearerToken });
                if (activeCookie && activeCookie.id === data.existingCookieId) {
                    setActiveCookie(prev => prev ? { ...prev, value: data.value, bearerToken: data.bearerToken } : null);
                }
                showToast('Cookie đã được cập nhật thành công!', 'success');
            } else {
                // Trường hợp THÊM MỚI
                const name = data.email ? `Cookie ${data.email}` : `Cookie ${new Date().toLocaleTimeString()}`;
                ...
                showToast(`Đã thêm và kích hoạt cookie mới: ${name}`, 'success');
            }
        });
